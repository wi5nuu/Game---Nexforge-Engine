...
impl RenderContext {
    ...
    pub fn new_with_size(size: (u32, u32)) -> Self {
        Self {
            size,
            ..Self::new()
        }
    ...
    pub fn resize(&mut self, new_size: (u32, u32)) {
        self.size = new_size;
        if let (Some(config), Some(device), Some(ref surface)) =
...
impl Default for ClearPipeline {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_render_context_creation() {
        ...
        assert_eq!(ctx.size, (1920, 1080));
    }

    #[test]
    fn test_clear_pipeline_creation() {
        let pipeline = ClearPipeline::new();
        assert!(true);
    }
}
