...
fn main() {
    ...
    log::info!("Nexforge Game starting with settings: {}x{} {}", settings.window_width, settings.window_height, settings.window_title);
    ...
    let window = match WindowBuilder::new()
        ...
        .with_inner_size(winit::dpi::LogicalSize::new(settings.window_width as f64, settings.window_height as f64))
        .build(&event_loop)
    {
        Ok(w) => w,
    ...
    if let Err(e) = engine.initialize() { log::error!("Engine init failed: {}", e); return; }
    ...
    let script_files = ["game/scripts/player.nxs", "game/scripts/enemy_ai.nxs", "game/scripts/weapon.nxs", "game/scripts/game_manager.nxs"];
    let loaded = script_files.iter().filter(|p| engine.load_script(p).is_ok()).count();
    log::info!("Loaded {}/{} scripts, {} entities", loaded, script_files.len(), engine.script.entities.len());

    if let Err(e) = engine.start() { log::error!("Engine start failed: {}", e); return; }
    ...
    event_loop.run(move |event, target| {
        ...
        match event {
            Event::WindowEvent { event: window_event, .. } => match window_event {
                WindowEvent::CloseRequested => { engine.shutdown(); target.exit(); }
                WindowEvent::KeyboardInput { event: key_event, .. } => {
                    ...
                    if let PhysicalKey::Code(code) = key_event.physical_key {
                        match code {
                            ...
                            KeyCode::Escape => { engine.shutdown(); target.exit(); }
